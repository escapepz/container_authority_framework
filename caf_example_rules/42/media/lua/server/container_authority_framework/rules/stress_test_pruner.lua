...
local function createDummyValidator(ruleId)
	...
	return function(context)
		...
		if SafeLogger.shouldLog and SafeLogger.shouldLog(10) then
			SafeLogger.log("[CAF] Stress test rule " .. ruleId .. " executed", 10)
		end
	end
end
...
return function()
	if not CAF then
		SafeLogger.log("[CAF] Error: CAF singleton missing during stress_test_pruner registration!", 50)
		return
	end
	...
	SafeLogger.log("[CAF] Stress Test Pruner: Registered 60 rules (priorities 1-60)", 30)
	SafeLogger.log(
	...
	Events.OnInitGlobalModData.Add(function()
		...
		if valInfo then
			SafeLogger.log(
				string.format("[CAF] STRESS TEST RESULT: Validation listeners = %d (expected: 25)", valInfo.listeners),
				30
			)

			if valInfo.listeners == 25 then
				SafeLogger.log("[CAF] STRESS TEST: PRUNER WORKING CORRECTLY ✓", 30)
			else
				SafeLogger.log(
					string.format("[CAF] STRESS TEST: PRUNER FAILED! Expected 25, got %d ✗", valInfo.listeners),
					50
				)
			end
		end
